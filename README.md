...
The pipeline produces all empirical results, tables, and figures. These are assembled into a hand-authored research paper (`paper/moltbook_humanlike.tex`) submitted to AIES 2026 under the title **"Auditing Linguistic Diversity and Anomaly in Autonomous Agent Populations"**.
...
├── paper/                    # Research paper (hand-authored)
│   ├── moltbook_humanlike.tex    # AIES 2026 submission source
│   ├── moltbook_humanlike.pdf    # Compiled PDF
│   └── figures/                  # Figures copied from outputs/analyze/figures/
├── data/
...
| `make paper`       | `08_generate_paper.py`     | Copy updated figures to `paper/figures/`; write run manifest |
...
├── moltbook_humanlike.pdf         # Compiled PDF
├── run_manifest.json              # Records environment when make paper last ran
└── figures/                       # Figures copied from outputs/analyze/figures/
```

## The research paper

The paper lives in `paper/` and is **hand-authored** — it is not auto-generated by the pipeline. This separation is intentional:

- The pipeline (`make all`) produces all empirical outputs: summary tables, figures, outlier scores, and statistical tests.
- The paper (`paper/moltbook_humanlike.tex`) is written by the researcher and cites those results. It is versioned in git alongside the code.
- `make paper` keeps the paper's figures up to date by copying the latest plots from `outputs/analyze/figures/` into `paper/figures/`. It does **not** modify the `.tex` file.

### Paper workflow

```
make analyze          # (re)generates tables and figures in outputs/analyze/
make paper            # copies updated figures into paper/figures/
cd paper && pdflatex moltbook_humanlike.tex && pdflatex moltbook_humanlike.tex
```

Run `pdflatex` twice so that cross-references (tables, figures, bibliography) resolve correctly. The compiled PDF is `paper/moltbook_humanlike.pdf`.
